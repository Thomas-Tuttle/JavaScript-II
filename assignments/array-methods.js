...
let fullName = runners.map((name) => {
    return{'first_name': name.first_name, 'last_name': name.last_name};
});
...
let largeShirts = runners.filter((newshirt) => {
    return newshirt.shirt_size === "L";
});
console.log(largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = runners.reduce((total, first_name) => {
    return total += first_name.donation;
}, 0);
